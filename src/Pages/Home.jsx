...
const Home = () => {
  ...
  useEffect(() => {
    if(user){
      axios.get(`https://carrentalbe-4lpr.onrender.com/singleuser/${user._id}`)
    .then((res)=>{
      ...
      if(activerides.length>0 || pastrides.length>0){
        axios.patch(`https://carrentalbe-4lpr.onrender.com/users/update/${user._id}`,
    ...
    .catch((err)=>{
     ...
     .then((res)=>{
      ...
      res.data.Cars.forEach((e)=>{
        ...
        axios.patch(`https://carrentalbe-4lpr.onrender.com/cars/updatecar/${e._id}`,
            {dates:newdates})
            .then((res)=>{
            //  console.log(res)
            })
