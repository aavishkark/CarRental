...
export const getCarsByCity=(data)=>(dispatch)=>{
   ...
   dispatch({type:GET_CARS_REQ})
   axios.get(`https://carrentalbe-production-0103.up.railway.app/cars/filtercars`,
   {headers:finalParams})
   .then((res)=>{
...
export const addCarToLikes=(data)=>(dispatch)=>{
    axios.get(`https://carrentalbe-production-0103.up.railway.app/users/singleuser/${data.user._id}`)
    .then((res)=>{
      ...
      if(!flag){
         ...
         axios.patch(`https://carrentalbe-production-0103.up.railway.app/users/update/${data.user._id}`,{favourite:fav})
         .then((res)=>{
         })
         .catch((err)=>{
            console.log(err)
